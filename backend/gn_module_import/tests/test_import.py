...
from geonature.tests import login
...
class TestImports:
    def test_import_permissions(self, client):
        ...
        assert(int(info_role.value_filter) == 1)
        ...
        info_role = cruved.get_herited_user_cruved_by_action("U")[0]
        assert(info_role)
        assert(int(info_role.value_filter) == 2)
        imprt.check_instance_permission(info_role)  # right as we still are author
        ...
        info_role = cruved.get_herited_user_cruved_by_action("U")[0]
        assert(info_role)
        assert(int(info_role.value_filter) == 3)
        imprt.check_instance_permission(info_role)


    def test_list_imports(self, client, imports, users):
        r = client.get(url_for('import.get_import_list'))
        assert r.status_code == Unauthorized.code
        login(client, 'noright_user')
        r = client.get(url_for('import.get_import_list'))
    ...
    def test_get_import(self, client):
        ...
        db.session.add(imprt)
        db.session.commit()
        r =  client.get(url_for('import.get_one_import', import_id=imprt.id_import))
        assert(r.status_code == Unauthorized.code)
        login(client)
        r =  client.get(url_for('import.get_one_import', import_id=imprt.id_import))
        assert(r.status_code == 200)
    ...
    def test_delete_import(self, client):
        ...
        r = client.delete(url_for('import.delete_import', import_id=imprt.id_import))
        assert(r.status_code == Unauthorized.code)
        login(client)
        r = client.delete(url_for('import.delete_import', import_id=imprt.id_import))
        assert(r.status_code == 200)
    ...
    def test_import_columns(self, client, datasets):
        login(client, 'user')
        ...
        r = client.get(url_for('import.get_import_columns_name', import_id=import_id))
        ...
        r = client.get(f'/import/imports/{import_id}/columns')
        assert(r.status_code == 200)
        data = r.get_json()
        assert('cd_nom' in data)

    def test_import_preview(self, client):
        r = client.get(url_for('import.preview_valid_data', import_id=0))
        assert(r.status_code == Unauthorized.code)  # FIXME
        login(client)
        r = client.get(url_for('import.preview_valid_data', import_id=1))
        assert(r.status_code == 404)
        # TODO validate response

    def test_import_values(self, client, datasets):
        r = client.get(url_for('import.get_import_values', import_id=1))
        assert(r.status_code == Unauthorized.code)
        login(client, 'user')
        ...
        r = client.post(url_for('import.set_import_field_mapping', import_id=import_id),
                        data={'id_field_mapping': fieldmapping.id_mapping})
        assert(r.status_code == 200)
        r = client.get(url_for('import.get_import_values', import_id=import_id))
        assert(r.status_code == 200)
        data = r.get_json()
        schema = {
            'definitions': jsonschema_definitions,
    ...
    def test_import_errors(self, client, imports):
        login(client, 'user')
        r = client.get(url_for('import.get_import_errors', import_id=imports['own_import'].id_import))
        assert(r.status_code == 200)
        errors = r.get_json()
        assert(len(errors) == 0)


    def test_import_invalid_file(self, client, datasets):
        login(client, 'user')
        ...
        r = client.post(url_for('import.decode_file', import_id=imprt.id_import), data=data)
        ...
        # r = client.get(url_for('import.get_import_errors', import_id=imprt.id_import))
        # assert(r.status_code == 200)
        # errors = r.get_json()
        # assert(len(errors) == 1)


    def test_import_valid_file(self, client, datasets):
        login(client, 'user')
        ...
        r = client.post(url_for('import.set_import_field_mapping', import_id=imprt.id_import), data=data)
        assert(r.status_code == 200)
        data = r.get_json()
        ...
        r = client.post(url_for('import.set_import_content_mapping', import_id=imprt.id_import), data=data)
        assert(r.status_code == 200)
        data = r.get_json()
        validate_json(data, {'definitions': jsonschema_definitions, '$ref': '#/definitions/import'})
        ...
        r = client.get(url_for('import.preview_valid_data', import_id=imprt.id_import))
        assert(r.status_code == 200)

        # Get invalid data
        r = client.get(url_for('import.get_import_invalid_rows_as_csv', import_id=imprt.id_import))
        assert(r.status_code == 200)
    ...
    def test_mapping_list(self, client, users):
        ...
        r = client.get(url_for('import.get_mapping_list'), query_string={'type': 'field'})
        assert(r.status_code == 403)

        login(client, 'self_user')
        ...
        assert(client.get(url_for('import.get_one_mapping', id_mapping=mapping2.id_mapping)).status_code == 200)
        assert(mapping3.id_mapping not in mapping_ids)  # not public
        assert(client.get(url_for('import.get_one_mapping', id_mapping=mapping3.id_mapping)).status_code == 403)
        assert(mapping4.id_mapping not in mapping_ids)  # not active
        assert(client.get(url_for('import.get_one_mapping', id_mapping=mapping4.id_mapping)).status_code == 403)
        assert(mapping5.id_mapping in mapping_ids)  # not public but user is owner
        assert(client.get(url_for('import.get_one_mapping', id_mapping=mapping5.id_mapping)).status_code == 200)
        assert(mapping6.id_mapping not in mapping_ids)  # not public and owned by another user
        assert(client.get(url_for('import.get_one_mapping', id_mapping=mapping6.id_mapping)).status_code == 403)
        ...
        assert(client.get(url_for('import.get_one_mapping', id_mapping=mapping7.id_mapping)).status_code == 403)
        ...
        assert(client.get(url_for('import.get_one_mapping', id_mapping=mapping7.id_mapping)).status_code == 200)
        ...
        assert(client.get(url_for('import.get_one_mapping', id_mapping=mapping6.id_mapping)).status_code == 200)


    def test_add_mapping(self, client):
        r = client.post(url_for('import.add_mapping'), data={'name': 'test', 'type': 'field'})
        assert(r.status_code == 401)
        login(client, 'user')
        r = client.post(url_for('import.add_mapping'), data={'name': 'test', 'type': 'invalid'})
        assert(r.status_code == 400)
        r = client.post(url_for('import.add_mapping'), data={'name': 'test', 'type': 'field'})
        assert(r.status_code == 200)
        mapping1 = r.get_json()
        ...
        r = client.post(url_for('import.add_mapping'), data={'name': '', 'type': 'field'})
        assert(r.status_code == 200)
    ...
    def test_rename_mapping(self, client):
        ...
        db.session.add(mapping2)
        db.session.commit()
        r = client.post(url_for('import.rename_mapping', id_mapping=mapping1.id_mapping),
                        data={'name': 'mapping2'})
        assert(r.status_code == 401)
        login(client)
        r = client.post(url_for('import.rename_mapping', id_mapping=mapping1.id_mapping),
    ...
    def test_delete_mapping(self, client):
        ...
        db.session.add(mapping5)
        db.session.commit()
        r = client.delete(url_for('import.delete_mapping', id_mapping=mapping1.id_mapping))
        assert(r.status_code == 401)
        login(client)
        r = client.delete(url_for('import.delete_mapping', id_mapping=mapping1.id_mapping))
        assert(r.status_code == BadRequest.code)
        r = client.delete(url_for('import.delete_mapping', id_mapping=mapping2.id_mapping))
        assert(r.status_code == 200)
        mappings = r.get_json()
        ...
        r = client.delete(url_for('import.delete_mapping', id_mapping=mapping4.id_mapping))
        assert(r.status_code == 200)
        mappings = r.get_json()
        validate_json(mappings, schema)
        mappings_ids = { mapping['id_mapping'] for mapping in mappings }
    ...
    def test_get_mapping_fields(self, client):
        mapping = TMappings(mapping_label="test", mapping_type="FIELD", active=True)
        db.session.add(mapping)
        db.session.commit()
        r = client.get(url_for('import.get_mapping_fields', id_mapping=mapping.id_mapping))
        assert(r.status_code == 401)
        login(client)
        r = client.get(url_for('import.get_mapping_fields', id_mapping=mapping.id_mapping))
        assert(r.status_code == 200)
    ...
    def test_update_mapping_fields(self, client):
        mapping = TMappings(mapping_label="test", mapping_type="FIELD", active=True)
        db.session.add(mapping)
        ...
        r = client.post(url_for('import.update_mapping_fields', id_mapping=id_mapping), data=mappings)
        assert(r.status_code == 401)
        login(client)
        r = client.post(url_for('import.update_mapping_fields', id_mapping=id_mapping), data=mappings)
        assert(r.status_code == 200)
    ...
    def test_get_mapping_contents(self, client):
        mapping = TMappings(mapping_label="test", mapping_type="CONTENT", active=True)
        db.session.add(mapping)
        db.session.commit()
        r = client.get(url_for('import.get_mapping_contents', id_mapping=mapping.id_mapping))
        assert(r.status_code == 401)
        login(client)
        r = client.get(url_for('import.get_mapping_contents', id_mapping=mapping.id_mapping))
        assert(r.status_code == 200)
    ...
    def test_update_mapping_contents(self, client):
        mapping = TMappings(mapping_label="test", mapping_type="CONTENT", active=True)
        db.session.add(mapping)
        ...
        #id_mapping = mapping.id_mapping  # save id mapping to re-use after session expunge
        #db.session.expunge_all()  # detach previous created object from db session to avoid merge conflict (merge is used in the view)
        r = client.post(url_for('import.update_mapping_contents', mapping_id=mapping.id_mapping), data=[])
        assert(r.status_code == 401)
        login(client)
        r = client.post(url_for('import.update_mapping_contents', mapping_id=mapping.id_mapping), data=[{'wrong': 'format'}])
        assert(r.status_code == BadRequest.code)
        data = [
        ...
        r = client.post(url_for('import.update_mapping_contents', mapping_id=mapping.id_mapping), data=data)
        assert(r.status_code == 200)
        data = r.get_json()
        validate_json(data, {
            'definitions': jsonschema_definitions,
    ...
    def test_synthesis_fields(self, client):
        assert(client.get(url_for('import.get_synthesis_fields')).status_code == 401)
        login(client)
        r = client.get(url_for('import.get_synthesis_fields'))
        assert(r.status_code == 200)
        data = r.get_json()

jsonschema_definitions = {
    'synthesis_field': {
        'type': 'object',
        'properties': {
            'id_field': { 'type': 'integer', },
            'name_field': { 'type': 'string', },
            'fr_label': { 'type': 'string', },
            'eng_label': { 'type': 'string', },
            'desc_field': { 'type': 'string', },
            'type_field': { 'type': 'string', },
            'synthese_field': { 'type': 'boolean', },
            'mandatory': { 'type': 'boolean', },
            'autogenerated': { 'type': 'boolean', },
            'nomenclature': { 'type': 'boolean', },
            'mnemonique': { 'type': ['string', 'null',], },
            'id_theme': { 'type': 'integer', },
            'order_field': { 'type': 'integer', },
            'comment': { 'type': ['string', 'null',], },
            'display': { 'type': 'boolean', },
        },
        'required': [
            'id_field',
            'name_field',
            'fr_label',
            'eng_label',
            'desc_field',
            'type_field',
            'synthese_field',
            'mandatory',
            'autogenerated',
            'nomenclature',
            'mnemonique',
            'id_theme',
            'order_field',
            'comment',
            'display',
        ],
        'additionalProperties': False,
    },
    'synthesis_theme': {
        'type': 'object',
        'properties': {
            'id_theme': { 'type': 'integer', },
            'name_theme': { 'type': 'string', },
            'fr_label_theme': { 'type': 'string', },
            'eng_label_theme': { 'type': 'string', },
            'desc_theme': { 'type': 'string', },
            'order_theme': { 'type': 'integer', },
        },
        'required': [
            'id_theme',
            'name_theme',
            'fr_label_theme',
            'eng_label_theme',
            'desc_theme',
            'order_theme',
        ],
        'additionalProperties': False,
    },
    'nomenclature_type': {
        'type': 'object',
        'properties': {
            'id_type': { 'type': 'integer', },
            'mnemonique': { 'type': 'string', },
            'label_default': { 'type': 'string', },
            'definition_default': { 'type': 'string', },
        },
        'required': [
            'id_type',
            'mnemonique',
            'label_default',
            'definition_default',
        ],
    },
    'nomenclature': {
        'type': 'object',
        'properties': {
            'cd_nomenclature': { 'type': 'string', },
            'mnemonique': { 'type': 'string', },
            'label_default': { 'type': 'string', },
            'definition_default': { 'type': 'string', },
            'source': { 'type': 'string', },
            'statut': { 'type': 'string', },
            'id_broader': { 'type': 'integer', },
            'hierarchy': { 'type': 'string', },
            'active': { 'type': 'boolean', },
            'meta_create_date': { 'type': 'string', },
            'meta_update_date': { 'type': 'string', },
        },
        'required': [
            'cd_nomenclature',
            'mnemonique',
            'label_default',
            'definition_default',
            'source',
            'statut',
            'id_broader',
            'hierarchy',
            'active',
        ],
    },
    'import': {
        'type': 'object',
        'properties': {
            'id_import': { 'type': 'integer' },
            'format_source_file': { 'type': ['string', 'null'] },
            'srid': { 'type': ['number', 'null'] },
            'separator': { 'type': ['string', 'null'] },
            'encoding': { 'type': ['string', 'null'] },
            'detected_encoding': { 'type': ['string', 'null'] },
            'import_table': { 'type': ['string', 'null'] },
            'full_file_name': { 'type': ['string', 'null'] },
            'id_dataset': { 'type': ['integer', 'null'] },
            'id_field_mapping': { 'type': ['integer', 'null'] },
            'id_content_mapping': { 'type': ['integer', 'null'] },
            'date_create_import': { 'type': 'string' },
            'date_update_import': { 'type': 'string' },
            'source_count': { 'type': ['integer', 'null'] },
            'import_count': { 'type': ['integer', 'null'] },
            'taxa_count': { 'type': ['integer', 'null'] },
            'date_min_data': { 'type': ['string', 'null'] },
            'date_max_data': { 'type': ['string', 'null'] },
            'uuid_autogenerated': { 'type': ['boolean', 'null'] },
            'is_finished': { 'type': ['boolean', 'null'] },
            'processing': { 'type': ['boolean', 'null'] },
            'in_error': { 'type': ['boolean', 'null'] },
            'authors_name': { 'type': 'string' },
            'available_encodings': { 'type': 'array', 'items': { 'type': 'string' } },
            'available_formats': { 'type': 'array', 'items': { 'type': 'string' } },
            'columns': {
                'type': ['null', 'object'],
                'additionalProperties': { 'type': 'string' },
            },
        },
        'required': [
            'id_import',
            'format_source_file',
            'srid',
            'separator',
            'encoding',
            'detected_encoding',
            'import_table',
            'full_file_name',
            'id_dataset',
            'id_field_mapping',
            'id_content_mapping',
            'date_create_import',
            'date_update_import',
            'source_count',
            'import_count',
            'taxa_count',
            'date_min_data',
            'date_max_data',
            'uuid_autogenerated',
            'is_finished',
            'processing',
            'in_error',
            'authors_name',
            'available_encodings',
            'available_formats',
        ],
    },
    'mapping': {
        'type': 'object',
        'properties': {
            'id_mapping': { 'type': 'integer' },
            'mapping_label': { 'type': 'string' },
            'mapping_type': {
                'type': 'string',
                'enum': [ 'FIELD', 'CONTENT'],
            },
            'active': { 'type': 'boolean' },
            'is_public': { 'type': 'boolean' },
            'cruved': { '$ref': '#/definitions/cruved' },
        },
        'minProperties': 6,
        'additionalProperties': False,
    },
    'mappingfield': {
        'type': 'object',
        'properties': {
            'id_match_fields': { 'type': 'integer', },
            'id_mapping': { 'type': 'integer', },
            'source_field': { 'type': 'string', },
            'target_field': { 'type': 'string', },
        },
        'minProperties': 4,
        'additionalProperties': False,
    },
    'mappingcontent': {
        'type': 'object',
        'properties': {
            'id_match_values': { 'type': 'integer', },
            'id_mapping': { 'type': 'integer', },
            'target_field_name': { 'type': 'string', },
            'source_value': { 'type': 'string', },
            'id_target_value': { 'type': 'integer', },
        },
        'minProperties': 5,
        'additionalProperties': False,
    },
    'cruved': {
        'type': 'object',
        'properties': {
            'C': { 'type': 'boolean' },
            'R': { 'type': 'boolean' },
            'U': { 'type': 'boolean' },
            'V': { 'type': 'boolean' },
            'E': { 'type': 'boolean' },
            'D': { 'type': 'boolean' },
        },
        'minProperties': 6,
        'additionalProperties': False,
    },
}

...
def extract(table_name, schema_name, column_names, index_col, imprt):
    ...
    index_dask = sqlalchemy.sql.column(index_col).label("gn_id")
    from ..utils.imports import get_import_table_name, get_table_class
    ImportTable = get_table_class(get_import_table_name(imprt))
    nb_row = db.session.query(ImportTable).count()
    npartition = 1 if nb_row < 50000 else 2

...
def manage_errors_and_validity(
    df, import_id, code_error, df_temp_col, column_invalid, id_rows_error
):
    """
...
def check_geography(
    df, import_id, selected_columns, srid, local_srid
):
    try:
        logger.info("CHECKING GEOGRAPHIC DATA:")
        file_srid_bounding_box = calculate_bounding_box(srid)

        mask_with_code = pd.Series(False, index=df.index)
        if 'codecommune' in selected_columns:
            mask_with_code |= df[selected_columns["codecommune"]].notnull()
        if 'codemaille' in selected_columns:
            mask_with_code |= df[selected_columns["codemaille"]].notnull()
        if 'codedepartement' in selected_columns:
            mask_with_code |= df[selected_columns["codedepartement"]].notnull()
        line_with_codes = df[mask_with_code].index

        print("setting given geom to null")
        df["given_geom"] = None
        ...
        if "latitude" and "longitude" in selected_columns.keys():
            ...
            # load wkt
            df["given_geom"] = df[selected_columns["WKT"]].apply(lambda x: wkt_to_wkb(x))

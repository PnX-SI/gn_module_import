...
def check_uuid(df,selected_columns,synthese_info):
    ...
    try:
        ...
        if len(uuid_cols) > 0:
            ...
            for col in uuid_cols:
                ...
                if col == 'unique_id_sinp':
                    ...
                    df[selected_columns[col]] = df[selected_columns[col]].apply(lambda x: fill_nan_uuid(x), meta=False)
        ...
        if 'unique_id_sinp' not in uuid_cols:
            df['unique_id_sinp'] = ''
            df['unique_id_sinp'] = df['unique_id_sinp'].apply(lambda x: str(uuid4()), meta=False)

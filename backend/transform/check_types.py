...
def check_types(df, selected_columns, synthese_info, missing_values):
    ...
    try:
        ...
        for field in date_fields:
            ...
            # datetime conversion
            df[selected_columns[field]] = df[selected_columns[field]].apply(lambda x: convert_to_datetime(x),meta=False)

            # check invalid date type and set user errors in db and front interface
            df['temp'] = ''
            df['temp'] = df['temp'].where(cond=df[selected_columns[field]].apply(lambda x: is_datetime(x),meta=False), other=False)
